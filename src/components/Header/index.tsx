...
const Header = () => {
  ...
  useEffect(() => {
    ...
    switch (true) {
      case currentPath.includes('challenges'):
        path = 'challenges'
        break
      case currentPath.includes('quests'):
        path = 'quests'
        break
      case currentPath.includes('admins'):
        path = 'admins'
        break
      case currentPath === '/':
        path = localStorage.getItem('lastTab') ?? 'challenges'
        break
      default:
        path = 'challenges'
    }
        ...
        <div className={styles.Header__TopBar}>
          ...
          <nav className={styles.Header__NavigationBar}>
            {navigationTabs.map((tab) => (
              <button key={tab.toLowerCase()} id={tab.toLowerCase()} onClick={onNavigationItemClick} className={styles.Header__NavigationItem}>{tab}</button>
            ))}
